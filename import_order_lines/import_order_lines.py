...
class order_line_wizard(models.TransientModel):
    ...
    def import_sol(self):
        ...
        for row_no in range(sheet.nrows):
            ...
            if row_no <= 0:
                ...
                if line[0]:
                    print("Internal Referance ======>",int(float(line[0])))
                    product_id = self.env['product.product'].search([('default_code','=',str(int(float(line[0]))))])
                    print("Internal Referance product_id ======>",product_id) 
                    if not product_id:
                         raise Warning(_('Product with "%s" Default code not in your system') % line[0])
                values.update({
    ...
    def create_order_line(self,values):
        ...
        uom=values.get('uom')
        product_obj_search=values.get('product_id')
